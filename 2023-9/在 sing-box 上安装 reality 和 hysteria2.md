...
```
注释1：实际使用的时候，我的pve上的虚拟机不支持 tun,也没有去深究原因，而且以前安装 zerotier-one也出问题，无法建立虚拟接口，所以放弃tun接口。直接把 inbounds -  "type": "tun" 这一部分删除掉。就可以 启动 客户端sing-box服务了。要不然启动总是失败。
注释2：inbound中接听部分记得从 127.0.0.1 改成 0.0.0.0 也就是监听所有的网络接口，这从局域网的流量也能代理啦
注释3：用 sock5代理，可以看 youtube,但是不能发表评论,这个问题已经发生很久了。使用ssh tunnel 可以看youtube也能发表评论。这其中的差别原因不明，估计要用全局代理tun模式会好一些吧，以后有机会再研究。
